...
// Helper Methods - Cards Function

void adventureCard(int currentPlayer, int z, int temphand[], struct gameState *state) {
    
    int drawntreasure=0;
    int cardDrawn;
    
    // ** Bug #1: Changed from 2 to 3
...
void councilRoomCard(int currentPlayer, int handPos, struct gameState *state) {
    int i;
    //+4 Cards
    for (i = 0; i < 4; i++)
    {
...
void feastCard(int currentPlayer, int choice1, int temphand[], struct gameState *state) {
    int i;
    int x;
    ...
    // Bug #2: Change the bonus coin from 5 to 7
    updateCoins(currentPlayer, state, 7);
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
      adventureCard(currentPlayer, z, temphand, state);
      return 0;
			
    case council_room:
      councilRoomCard(currentPlayer, handPos, state);
      return 0;
			
    case feast:
      feastCard(currentPlayer, choice1, temphand, state);
      return 0;
